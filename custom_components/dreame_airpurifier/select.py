...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    for p in data["purifiers"]:
        entities.extend([
            DreameLightControlSelect(data["coordinator"], p),
            DreameVoiceInteractionVolumeSelect(data["coordinator"], p),
        ])
...
class DreameLightControlSelect(DreameBaseSelect):
    ...
    async def async_select_option(self, option: str) -> None:
        value = LIGHT_CONTROL_OPTIONS[option]
        await self.hass.async_add_executor_job(self._purifier.set_light_control, value)
...
class DreameVoiceInteractionVolumeSelect(DreameBaseSelect):
    ...
    async def async_select_option(self, option: str) -> None:
        value = VOICE_INTERACTION_VOLUME_OPTIONS[option]
        await self.hass.async_add_executor_job(self._purifier.set_voice_interaction_volume, value)
        await self.coordinator.async_request_refresh()
